...
variable "lambda_function_architecture" {
  description = "The architecture of the Lambda function. Valid values are x86_64 or arm64"
  type        = string
  default     = "x86_64"

  validation {
    condition     = contains(["x86_64", "arm64"], var.lambda_function_architecture)
    error_message = "The lambda_function_architecture value must be either 'x86_64' or 'arm64'."
  }
}
